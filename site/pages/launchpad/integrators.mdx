...
| `startTick` | Initial SushiSwap V3 pool tick derived from the launch's starting FDV. |
| `poolInitializedAt` | Pool initialization time as a Unix timestamp in seconds. |
| `observationCardinalityNext` | V3 oracle observation capacity configured when the pool was initialized. |
| `name` | Launch token name. |
| `symbol` | Launch token symbol. |

The first three fields are indexed event topics; all remaining fields are
ABI-encoded in the log data. Treat `(transactionHash, logIndex)` as the event's
stable identifier, persist the block number and block hash, and account for
chain reorganizations before considering a launch final. After discovering a
token, use `launchpad.token` to hydrate current mutable fields such as
`creator`, `feeReceiver`, `feeDisposition`, metadata, and market data.
