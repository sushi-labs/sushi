...
import { ROUTE_PROCESSOR_9_ADDRESS } from 'sushi/evm'
...
function processRoute(
  ...
  bool takeSurplus,
  uint32 referralCode
) public payable returns (uint256 amountOut, bytes memory diagnostics)
```
...
| `diagnostics` | `bytes`   | Encoded per-pool slippage diagnostics |
...
function processRouteWithTransferValueInput(
  ...
  bool takeSurplus,
  uint32 referralCode
) external payable returns (uint256 amountOut, bytes memory diagnostics)
```
...
| `diagnosticsFirst32` | `bytes32` | First 32 bytes of diagnostics data                              |

## Deployments

<AddressTable data={ROUTE_PROCESSOR_9_ADDRESS} />
