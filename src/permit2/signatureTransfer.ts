...
export abstract class SignatureTransfer {
  ...
  >(
    ...
    invariant(
      MaxSigDeadline >= BigInt(permit.deadline),
  ...
  public static hash(
    ...
    const { domain, types, values, primaryType } =
      SignatureTransfer.getPermitData(permit, permit2Address, chainId, witness)
...
function validateTokenPermissions(permissions: TokenPermissions) {
  invariant(
    MaxSignatureTransferAmount >= BigInt(permissions.amount),
    'AMOUNT_OUT_OF_RANGE',
  )
}
