...
export type PermitSingleData = TypedStructData<
  typeof PERMIT_TYPES,
  PermitSingle,
  'PermitSingle'
>
export type PermitBatchData = TypedStructData<
  typeof PERMIT_BATCH_TYPES,
  PermitBatch,
  'PermitBatch'
>
...
export abstract class AllowanceTransfer {
  ...
  public static getPermitData<
    ...
    ReturnType = TPermit extends PermitSingle
      ? PermitSingleData
      : PermitBatchData,
  >(
    permit: TPermit,
    ...
    invariant(
      MaxSigDeadline >= BigInt(permit.sigDeadline),
  ...
  public static hash(
    ...
    const { domain, types, values, primaryType } =
      AllowanceTransfer.getPermitData(permit, permit2Address, chainId)
...
function validatePermitDetails(details: PermitDetails) {
  ...
  invariant(
    MaxAllowanceTransferAmount >= BigInt(details.amount),
    'AMOUNT_OUT_OF_RANGE',
  )
  invariant(
    MaxAllowanceExpiration >= BigInt(details.expiration),
