...
describe('NonfungiblePositionManager', () => {
  ...
  const tokenId = 1n
  const slippageTolerance = new Percent({ numerator: 1, denominator: 100 })
  const deadline = 123
  ...
  describe('#addCallParameters', () => {
      ...
      expect(() =>
        NonfungiblePositionManager.addCallParameters(
          ...
          }),
          {
            ...
            slippageTolerance,
            deadline,
            useNative: EvmNative.fromChainId(1),
          },
        ),
      ).toThrow('NO_WETH')
    })
    ...
    it('useNative', () => {
      ...
      expect(value).toEqual('0x1')
    })
  })

  describe('#collectCallParameters', () => {
    const token2 = new EvmToken({
      chainId: 1,
    ...
    it('works', () => {
      const { calldata, value } =
        NonfungiblePositionManager.collectCallParameters({
          tokenId,
          expectedCurrencyOwed0: new Amount(token0, 0),
          expectedCurrencyOwed1: new Amount(token1, 0),
          recipient,
        })
    ...
    it('works with eth', () => {
      const { calldata, value } =
        NonfungiblePositionManager.collectCallParameters({
          tokenId,
          expectedCurrencyOwed0: new Amount(token1, 0),
          expectedCurrencyOwed1: new Amount(EvmNative.fromChainId(1), 0),
          recipient,
        })
      ...
      const { calldata, value } =
        NonfungiblePositionManager.collectCallParameters([
          ...
          },
          {
            tokenId: 2,
            expectedCurrencyOwed0: new Amount(token0, 0),
            expectedCurrencyOwed1: new Amount(token2, 0),
            recipient,
          },
        ])
    ...
    it('aggregates multiple eth collections', () => {
      const { calldata, value } =
        NonfungiblePositionManager.collectCallParameters([
          {
            tokenId: 1,
            expectedCurrencyOwed0: new Amount(token0, 1n),
            expectedCurrencyOwed1: new Amount(EvmNative.fromChainId(1), 1n),
            recipient,
          },
          {
            tokenId: 2,
            expectedCurrencyOwed0: new Amount(token1, 2n),
            expectedCurrencyOwed1: new Amount(EvmNative.fromChainId(1), 2n),
            recipient,
          },
          {
            tokenId: 3,
            expectedCurrencyOwed0: new Amount(token2, 3n),
            expectedCurrencyOwed1: new Amount(EvmNative.fromChainId(1), 3n),
            recipient,
          },
        ])
    ...
    it('aggregates tokens and eth by recipient', () => {
      ...
      const { calldata, value } =
        NonfungiblePositionManager.collectCallParameters([
          ...
          {
            tokenId: 2,
            expectedCurrencyOwed0: new Amount(EvmNative.fromChainId(1), 4n),
            expectedCurrencyOwed1: new Amount(token0, 3n),
            recipient: recipient,
          },
          {
            tokenId: 3,
            expectedCurrencyOwed0: new Amount(token1, 5n),
            expectedCurrencyOwed1: new Amount(EvmNative.fromChainId(1), 6n),
            recipient: recipient2,
          },
          {
            tokenId: 4,
            expectedCurrencyOwed0: new Amount(EvmNative.fromChainId(1), 8n),
            expectedCurrencyOwed1: new Amount(token1, 7n),
            recipient: recipient2,
          },
        ])
  ...
  describe('#removeCallParameters', () => {
    ...
    it('works for partial', () => {
      const { calldata, value } =
        NonfungiblePositionManager.removeCallParameters(
          ...
          {
            tokenId,
            liquidityPercentage: new Percent({ numerator: 1, denominator: 2 }),
            slippageTolerance,
            deadline,
            collectOptions: {
              expectedCurrencyOwed0: new Amount(token0, 0),
    ...
    it('works with eth', () => {
      const ethAmount = new Amount(EvmNative.fromChainId(1), 0)
      const tokenAmount = new Amount(token1, 0)
      ...
      const { calldata, value } =
        NonfungiblePositionManager.removeCallParameters(
          ...
          }),
          {
            ...
            deadline,
            collectOptions: {
              expectedCurrencyOwed0: pool_1_weth.token0.isSame(token1)
                ? tokenAmount
                : ethAmount,
              expectedCurrencyOwed1: pool_1_weth.token0.isSame(token1)
                ? ethAmount
                : tokenAmount,
              recipient,
            },
