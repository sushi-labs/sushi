...
export class SushiSwapV3Pool {
  public readonly token0: EvmToken
  public readonly token1: EvmToken
  ...
  private _token0Price?: Price<EvmToken, EvmToken>
  private _token1Price?: Price<EvmToken, EvmToken>
  ...
  public get token0Price(): Price<EvmToken, EvmToken> {
    if (!this._token0Price) {
      this._token0Price = new Price({
        ...
        quote: this.token1,
        denominator: Q192,
        numerator: this.sqrtRatioX96 * this.sqrtRatioX96,
      })
    }
  ...
  public get token1Price(): Price<EvmToken, EvmToken> {
    if (!this._token1Price) {
      this._token1Price = new Price({
        base: this.token1,
        quote: this.token0,
        denominator: this.sqrtRatioX96 * this.sqrtRatioX96,
        numerator: Q192,
      })
    }
  ...
  public async getOutputAmount(
  ...
  ): Promise<[Amount<EvmToken>, SushiSwapV3Pool]> {
    invariant(this.involvesToken(inputAmount.currency), 'TOKEN')

    const zeroForOne = inputAmount.currency.isSame(this.token0)
    ...
    const {
      ...
      liquidity,
      tickCurrent,
    } = await this.swap(zeroForOne, inputAmount.amount, sqrtPriceLimitX96)
    const outputToken = zeroForOne ? this.token1 : this.token0
    return [
      new Amount(outputToken, outputAmount * -1n),
      new SushiSwapV3Pool(
        this.token0,
        this.token1,
        this.fee,
  ...
  public async getInputAmount(
    ...
    invariant(
      outputAmount.currency.type === 'token' &&
        this.involvesToken(outputAmount.currency),
      'TOKEN',
    )

    const zeroForOne = outputAmount.currency.isSame(this.token1)
    ...
    const {
      ...
      zeroForOne,
      outputAmount.amount * -1n,
      sqrtPriceLimitX96,
    )
    const inputToken = zeroForOne ? this.token0 : this.token1
    return [
      new Amount(inputToken, inputAmount),
      new SushiSwapV3Pool(
        this.token0,
        this.token1,
        this.fee,
