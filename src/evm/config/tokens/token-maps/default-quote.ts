...
import { getEvmNativeByChainId, type EvmNative } from '~evm/currency/native.js'
import { createEvmToken } from '~evm/currency/token.js'
import { WNATIVE } from '../wrapped-native.js'

export const defaultCurrency = {
  ...(Object.fromEntries(
    evmChainIds.map((chainId) => [chainId, getEvmNativeByChainId(chainId)]),
...
export const defaultQuoteCurrency = {
  ...
  [EvmChainId.BOBA_BNB]: createEvmToken({
  ...
  [EvmChainId.CURTIS]: USDT[EvmChainId.CURTIS],
  [EvmChainId.BSC_TESTNET]: WNATIVE[EvmChainId.BSC_TESTNET],
  [EvmChainId.FANTOM_TESTNET]: WNATIVE[EvmChainId.FANTOM_TESTNET],
} as const satisfies Record<EvmChainId, EvmCurrency>
