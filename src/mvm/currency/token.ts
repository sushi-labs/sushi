...
>({
  ...
  z.object({
    chainId: z.number().int().refine(isMvmChainId).transform((chainId) => chainId as MvmChainId),
    address: z.string().refine(isMvmAddress).transform((address) => address as MvmAddress),
    symbol: z.string(),
    name: z.string(),
