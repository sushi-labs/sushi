...
import { BaseCurrency, type Currency, type CurrencyMetadata } from "./currency.js";
...
describe("generic/currency/currency.ts types", () => {
	describe("id", () => {
		...
		it("should return the correct id for EvmToken", () => {
			...
			expectTypeOf(evmMockToken.id).toEqualTypeOf<`${EvmChainId}:${EvmAddress}`>();
		});
	});
	...
	describe("isNative/isToken", () => {
		it("should correctly narrow the type based on isNative", () => {
			...
			expectTypeOf(mockCurrency).extract<{ isNative: true }>().toEqualTypeOf<Native>();
			expectTypeOf(mockCurrency).extract<{ isNative: false }>().toEqualTypeOf<Token>();

			const mockEvmCurrency = {} as EvmCurrency;

			expectTypeOf(mockEvmCurrency).extract<{ isNative: true }>().toEqualTypeOf<EvmNative>();
			expectTypeOf(mockEvmCurrency).extract<{ isNative: false }>().toEqualTypeOf<EvmToken>();
		});

		it("should correctly narrow the type based on isToken", () => {
			const evmMockCurrency = {} as EvmCurrency;

			expectTypeOf(evmMockCurrency).extract<{ isToken: true }>().toEqualTypeOf<EvmToken>();
			expectTypeOf(evmMockCurrency).extract<{ isToken: false }>().toEqualTypeOf<EvmNative>();

			const mockEvmCurrency = {} as EvmCurrency;

			expectTypeOf(mockEvmCurrency).extract<{ isToken: true }>().toEqualTypeOf<EvmToken>();
			expectTypeOf(mockEvmCurrency).extract<{ isToken: false }>().toEqualTypeOf<EvmNative>();
		});
	});
	...
	describe("currency type extension", () => {
		...
		class CustomToken<TMetadata extends CurrencyMetadata = Record<string, unknown>>
			extends BaseCurrency<EvmChainId, TMetadata, "vault-token", SerializedCustomToken>
			implements Omit<Token<EvmChainId, CustomAddress>, "type" | "isToken" | "wrap" | "toJSON">
		{
			...
			constructor({
				...
				ConstructorParameters<typeof Token<EvmChainId, CustomAddress, TMetadata>>[0],
				"address"
			>) {
				super({ ...rest });
		...
		it("should narrow to the correct type", () => {
			...
			expectTypeOf(customToken).extract<{ type: "vault-token" }>().toEqualTypeOf<CustomToken>();
		});
		...
		it("should have the correct id", () => {
			...
			expectTypeOf(customToken.id).toEqualTypeOf<`${EvmChainId}:${EvmAddress}-${EvmAddress}`>();
	...
	describe("serialization", () => {
		describe("SerializedEvmToken", () => {
			...
			it("should infer metadata type from toJSON", () => {
				...
				expectTypeOf(tokenWithDefaultMetadata.toJSON().metadata).toEqualTypeOf<Record<string, unknown>>();
				...
				expectTypeOf(tokenWithCustomMetadata.toJSON().metadata).toEqualTypeOf<CustomMetadata>();
			});
		});
